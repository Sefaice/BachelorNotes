...
同步关系：直接制约关系，如生产和使用的关系；互斥关系：简介制约关系，不能同时访问一个临界资源。
...
管程的局部数据只能被管程的局部程序访问，即私有变量；进程只能通过调用管程内的过程访问共享数据，即共有变量只能通过调用方法访问；一次只能有一个进程在管程内执行某个内部过程。
...
目录结构：
* 单级目录结构
* 两级目录结构：第一级是主文件目录，记录用户名和响应用户文件目录位置，第二级是用户文件目录
* 多级目录结构/树形目录结构
