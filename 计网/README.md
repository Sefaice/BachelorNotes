...
* Identification标识
* Flags标志：3bits，第一位保留（没用），后两位是MF和DF，MF（More Fragment）= 1表示后续还有分片，DF（Don’t Fragment）= 0对数据报进行分片
...
子网掩码：为了表示对哪几位进行了子网划分，引入了子网掩码，地址一样长32bits。子网掩码1对应的位数为网络号及子网号，0对应位数为主机号。
...
* 所有结点的信息一致
* 只交换相邻结点的链路信息，与全局状态无关，因此网络规模大时OSPF协议比RIP协议效果好图
* 收敛快
