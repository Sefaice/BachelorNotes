...
局域网（Local Area Network，LAN）是一个叫较小范围内的网络。目前使用范围最广的局域网是Ethernet以太网。

IEEE 802标准定义的局域网对应于OSI参考模型的数据链路层和物理层，并将数据链路层拆为两个子层：LCC逻辑链路控制子层和MAC介质访问控制子层，LCC子层在上（王道p103）。
...
* **网卡**：
  * 又称网络适配器（Adapter）或网络接口卡（Network Interface Card，NIC）
...
* 前导码：8B。以太网帧不需要帧结束符，因为物理层传送时帧之间有一定间隙，没有帧结束符不等于没有数据链路层添加的尾部。
* 目的地址，源地址：共12B，**目的地址在前**。
* 类型：2B，指出携带的数据应交给哪个协议处理。
* 数据：**46B~1500B（带上首尾64B~1518B）**，数据长度不足46B时，必须填充到46B。
