...
#### distance-vector距离-向量算法

典型的是RIP（Routing Information Protocol）协议：
* 路由表条目：目的网络，下一跳路由器，跳数
* 定期或结点更新时与**相邻结点**交换路由表
* metrices跳数是抽象距离，即源端口到目的端口经过的路由个数，直连设备跳数为0，跳数16视为不可达
* 收到相邻结点发来的RIP报文时，对于报文中的路由条目：
  * 若该目的网络不存在路由表中，将路由加入路由表
  * 目的网络相同，下一跳地址相同，不论是否更短，都要更新路由表
...
典型的是OSPF协议：
* 链路状态变化时，将路由信息广播给**所有其他结点**，采用Flooding洪泛法广播
...
* 首部长度：4bits，分组首部的长度，**单位是4B**
* 总长度：2B，**单位是1B**，当封装成帧时，最大长度为1500B
* 标识
* 标志：3bits，第一位保留（没用），后两位是MF和DF，MF（More Fragment）= 1表示后续还有分片，DF（Don’t Fragment）= 0对数据报进行分片
* 片偏移：13bits，分片后的偏移量，只算数据长度，不包含首部，**单位是8B**
* 首部校验和：只校验分组首部，不校验数据

IP数据报分片：当分组长度大于数据链路层的MTU时，需要进行分片，分片有可能发生在发送端和发送过程中，**不同网络的MTU不同，因此可能在发送中多次分片，但只会在目的主机重组**。
...
<网络号>, <主机号>地址格式，路由器仅根据网络号转发分组，不考虑主机号。

王道p147，不同类别网络的地址范围、可用数目。
