...
* 端口号：长16bit，只有本地意义，不同计算机可以有相同的端口号，按端口号范围分为两类：
  * 服务端使用的端口号：
    ...
    * 登记端口号：1024~49151
  * 客户端使用的端口号：59152~65535，仅在客户端运行时才会动态选择
...
* 同步位SYN
* 终止位FIN
* 窗口字段：指允许对方发送的数据量，即接收方的接收缓存大小，发送方以此确定发送窗口的大小
...
* `ACK = 1, seq = u+1, ack = w+1`，客户端收到服务器释放连接的请求，发送确认，**此时TCP连接还未释放，等待计时器设置的2MSL时间后，连接才关闭**
...
* 网络拥塞处理：当出现超时，认为网络出现拥塞，让ssthresh = cwnd/2（但不能小于2），cwnd = 1，执行慢开始算法
