...
**机器字长，指令字长和存储字长**：
* 机器字长：32位计算机/64位计算机就是指机器字长，指计算机进行一次运算能处理的二进制数据的位数，一般等于内部寄存器的大小，它的大小决定的计算机的运算精度。
* 指令子长：计算机内指令的位数。
* 存储字长：存储单元的位数。
* 它们的长度都是字节的整数倍。
* 操作系统的位数指地址空间的大小，比如32位操作系统可以访问2^32 = 4G的地址空间，是软件的概念，而32位机器是硬件概念。64位操作系统一定要64位机器，但64位机器可以运行32位操作系统。
...
边界对齐：CMU课件note3，其实是一种空间换时间的思想，适应指令流水线。

#### 校验码

校验码是能发现或自动纠正错误的数据编码，也叫检错纠错码。原理是通过在原数据码上增加冗余码来达到检错/纠错。

**码距**：任意两个合法码字之间最少变化的二进制位数（位数，不是差值）。码距>=2时，校验码开始具有检错能力。码距越大，检错、纠错能力越强，且检错能力总是大于纠错能力，具体而言，若要能检测`d`位错误，码距至少为`d+1`，若要能纠正`d`位错误，码距至少为`2d+1`，见王道计网p70。
...
**CRC码/多项式编码**：可以检测并纠正1位错误。使用步骤：生成多项式G的最高次幂为校验位R的位数 - 原信息码左移R位 - **模2除**得到R位余数作为校验位 - 校验。
...
按存取方式分类：
* RAM随机存取存储器，断电信息消失（易失性存储器），SRAM做cache，DRAM做主存（**主存由RAM和ROM组成，引导程序放在ROM中**）。
* ROM只读存储器，非易失性存储器。广义上有些派生的ROM存储器已可以写入。RAM和ROM都是**随机存取**的，只是ROM写入一次之后不能再修改。
...
DRAM采用**地址复用**技术，地址线是原来的1/2，地址信号分行、列传送。且DRAM需要不断刷新，有三种刷新方式，[参考](https://zhidao.baidu.com/question/342245059.html)：
...
位扩展，字扩展。位扩展后以一个字为最小读写单位，所以地址区分不出组成一个字的子芯片了。
...
CPU与cache或主存以**字**为单位交换信息（CPU参与的读写都是以字为单位），cache与主存以**块**为单位交换信息。
...
**cache-主存系统的效率**：`cache存取周期 / cache-主存系统的平均访问时间`。
...
页式虚拟存储器：地址转换过程。页表在主存中，用于虚页号到实页号的转换。发生缺页时，先从辅存调入页，再重新进行页表访问，前后一共3次访存。
...
* 字段直接编码：把微命令字段分成小段，互斥的微命令放在同一字段，相容的放在不同字段，每个字段编码单独定义，因此长度缩短，需要译码。（**每段还要留出一个空状态，表示该字段不发出任何微命令。**）
...
流水线的效率：占时空图的有效面积（数格子，且必须数指令**不在一行**的时空图的格子，否则算出来错的），最高效率。
...
IO通道是专门负责输入/输出的处理机，是DMA方式的发展，可进一步减少CPU干预，通道运行通道程序完成IO，通道程序在内存中。

与DMA方式对比：
* DMA方式用CPU控制传输数据大小、内存位置，而通道方式是由通道控制的
* 每个DMA控制器对应一台设备，一个通道可以控制多个设备

**IO子系统的层次结构**，王道操作系统P277图5.3：
* 用户层IO软件
* 设备独立性软件：与硬件无关的工作，公有操作等
* 设备驱动程序：与硬件直接相关
* 中断处理程序
* 硬件：包括设备和**设备控制器**，王道P279图5.4
  * 设备控制器与CPU的接口
  * 设备控制器与设备的接口
  * IO控制逻辑
